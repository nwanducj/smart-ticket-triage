...
const CATEGORY_KEYWORDS: Record<TicketCategory, string[]> = {
  ...
  feature_request: [
    ...
    'dark mode', 'export', 'import', 'api', 'customize',
  ],
  account: [
    ...
    'register', 'deactivate', 'verify', 'verification',
  ],
  technical_support: [
 ...
 */
const PRIORITY_KEYWORDS: Record<'critical' | 'high' | 'low', string[]> = {
  critical: [
 ...
 */
function determinePriority(text: string): TicketPriority {
